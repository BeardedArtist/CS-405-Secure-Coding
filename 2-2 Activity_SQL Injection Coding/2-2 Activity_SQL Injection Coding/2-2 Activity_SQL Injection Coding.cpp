...
{
    // TODO: Fix this method to fail and display an error if there is a suspected SQL Injection (Completed)

    //Zane Solution CODE HERE... 
    //Resource used: https://en.cppreference.com/w/cpp/regex
    //Characters to be checked was tested using a Regex Tester
    //Regex is used to search through the array in a string and attempt to match the characters with the user input. 
    //If any of the characters match, then the program will through the warningMessage variable letting the user know that a SQL Injection has occured.

    std::regex GetSQLInjection("[^0-9A-Z+$]\\s*(and|or)\\s*[\\'\"]{0,1}", std::regex_constants::icase);
    if (std::regex_search(sql, GetSQLInjection)) 
    {
        std::string warningMessage = std::string("A SQL INJECTION HAS BEEN FOUND. TAKE IMMEDIATE ACTION");
        std::cout << "Failure! ERROR = " << warningMessage << std::endl;
        return false;
    }
    //END Zane Solution CODE... 
    //Resource used: https://en.cppreference.com/w/cpp/regex
...
int main()
{
    ...
    int return_code = 0;
    std::cout << "SQL Injection Example: By Zane Brown" << std::endl;
